...
int read_boot_info(struct boot_info *b)
{
    ...
    while (pos < end) {
            ...
            switch (tag->type) {
    /* loads initrd size and address info into boot_info for use in kernel.c*/
        case MULTIBOOT_TAG_TYPE_MODULE: {
            const struct multiboot_tag_module *m =
                (const struct multiboot_tag_module *)tag;

            b->initrd_addr = (void *)(uintptr_t)m->mod_start;
            b->initrd_size = (size_t)(m->mod_end - m->mod_start);
